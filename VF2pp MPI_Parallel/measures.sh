...
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 <#ofExecution> <#ofWorkers> <Graphs file name> [OPTIONAL: OPTIMIZATION] [OPTIONAL: METRICS_FILE][OPTIONAL: VERBOSE]"
    exit 1
fi

NUM_RUNS="$1"
WORKERS="$2"
if [ "$WORKERS" -lt 2 ] || [ "$WORKERS" -gt 8 ]; then
	echo "ERROR: #ofWorkers must be an integer in the range [2,8]."
	exit 1
fi
GRAPHS_FILENAME="$3"
...
echo "Run,Rank,Time" >> "$OUTPUT_FILE"

for((i = 1; i <= NUM_RUNS; i++))
do

	if [ "$VERBOSE" == "0" ]; then
		OUTPUT=$(mpirun -n "$WORKERS" ./VF2pp_parallel "$GRAPHS_FILENAME" 2>> "$LOG_FILE")
	else
		OUTPUT=$(mpirun -n "$WORKERS" ./VF2pp_parallel "$GRAPHS_FILENAME" "$VERBOSE" 2>> "$LOG_FILE")
	fi
